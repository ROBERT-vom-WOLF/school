...
class App:
    def __init__(self):
        ...
        self.notifier = Notifier(self.config["SENDER_EMAIL_ADDRESS"], self.config["SENDER_EMAIL_PASSWD"], self.config["RECEIVER_EMAIL_ADDRESS"], log_file_path)
    ...
    def mainloop(self):
        ...
        while True:
            ...
            print(f"({water_level}: {self.active_value})")
            user_input = input("Sensor Value: ").lower().strip()
